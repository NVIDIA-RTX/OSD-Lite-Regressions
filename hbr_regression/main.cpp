...
static Shape * readShape( std::filesystem::path const& filepath, Scheme scheme ) {

    std::string fname = filepath.lexically_normal().generic_string();

    FILE * handle = std::fopen(fname.c_str(), "rb");
    if (! handle) {
    ...
    if ( std::fread( shapeStr, size, 1, handle)!=1 ) {
