...
HbrCatmarkSubdivision<T>::transferFVarToChild(HbrMesh<T>* mesh, HbrFace<T>* face, HbrFace<T>* child, int index) {
    ...
    for (int fvaritem = 0; fvaritem < fvarcount; ++fvaritem) {
        ...
        if (fvarinterp == HbrMesh<T>::k_InterpolateBoundaryEdgeAndCorner) {
            if (fvarmask >= float(HbrVertex<T>::Mask::k_Corner)) {
                infcorner = true;
        ...
        if (fvarinterp == HbrMesh<T>::k_InterpolateBoundaryNone ||
            (fvarinterp == HbrMesh<T>::k_InterpolateBoundaryAlwaysSharp &&
             fvarmask >= 1) ||
            v->GetSharpness() > float(HbrVertex<T>::Mask::k_Smooth) ||
            infcorner) {
...
HbrCatmarkSubdivision<T>::Refine(HbrMesh<T>* mesh, HbrFace<T>* face) {
    ...
    for (int i = 0; i < nv; ++i) {
        if (!face->GetChild(i)) {
            ...
            assert(childedge);
            if ((sharpness = edge->GetSharpness()) > float(HbrHalfedge<T>::Mask::k_Smooth)) {
            ...
            assert(childedge);
            if ((sharpness = prevedge->GetSharpness()) > float(HbrHalfedge<T>::Mask::k_Smooth)) {
                HbrSubdivision<T>::SubdivideCreaseWeight(
...
HbrCatmarkSubdivision<T>::HasLimit(HbrMesh<T>* mesh, HbrHalfedge<T>* edge) {
    ...
    if (edge->GetSharpness() >= float(HbrHalfedge<T>::Mask::k_InfinitelySharp)) return true;
...
HbrCatmarkSubdivision<T>::HasLimit(HbrMesh<T>* /* mesh */, HbrVertex<T>* vertex) {
    ...
    switch (typename HbrVertex<T>::Mask(vertex->GetMask(false))) {
        case HbrVertex<T>::Mask::k_Smooth:
        case HbrVertex<T>::Mask::k_Dart:
            return !vertex->OnBoundary();
            break;
        case HbrVertex<T>::Mask::k_Crease:
        case HbrVertex<T>::Mask::k_Corner:
        default:
            if (vertex->IsVolatile()) {
                ...
                edge = start;
                while (edge) {
                    if (edge->IsBoundary() && edge->GetSharpness() < float(HbrHalfedge<T>::Mask::k_InfinitelySharp)) {
                        return false;
                    }
...
HbrCatmarkSubdivision<T>::Subdivide(HbrMesh<T>* mesh, HbrVertex<T>* vertex) {
    ...
    for (int i = 0; i < passes; ++i) {
        switch (typename HbrVertex<T>::Mask(masks[i])) {
            case HbrVertex<T>::Mask::k_Smooth:
            case HbrVertex<T>::Mask::k_Dart: {
    ...
    if (sharp >= float(HbrVertex<T>::Mask::k_InfinitelySharp)) {
        v->SetSharpness(float(HbrVertex<T>::Mask::k_InfinitelySharp));
    } else if (sharp > float(HbrVertex<T>::Mask::k_Smooth)) {
        sharp -= 1.0f;
        if (sharp < (float) HbrVertex<T>::Mask::k_Smooth) {
            sharp = (float) HbrVertex<T>::Mask::k_Smooth;
        }
        v->SetSharpness(sharp);
    } else {
        v->SetSharpness(float(HbrVertex<T>::Mask::k_Smooth));
    }
    return v;
}
