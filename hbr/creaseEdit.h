...
public:
    ...
    virtual void ApplyEditToFace(HbrFace<T>* face) {
        if (HbrHierarchicalEdit<T>::GetNSubfaces() == face->GetDepth()) {
            ...
            if (sharp < float(HbrHalfedge<T>::Mask::k_Smooth))
                sharp = float(HbrHalfedge<T>::Mask::k_Smooth);
            if (sharp > float(HbrHalfedge<T>::Mask::k_InfinitelySharp))
                sharp = float(HbrHalfedge<T>::Mask::k_InfinitelySharp);
