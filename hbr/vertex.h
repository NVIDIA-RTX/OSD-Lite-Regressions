...
HbrVertex<T>::IsFVarSmooth(int datum) {
    return (GetFVarMask(datum) == (unsigned char)Mask::k_Smooth);
}
...
HbrVertex<T>::IsFVarDart(int datum) {
    return (GetFVarMask(datum) == (unsigned char)Mask::k_Dart);
}
...
HbrVertex<T>::GetMask(bool next) {
    ...
    // Mark volatility
    if (sharpness > float(Mask::k_Smooth) && sharpness < float(Mask::k_InfinitelySharp))
        volatil = 1;

    // If the vertex is tagged as sharp immediately promote its mask
    // to corner
    if (IsSharp(false)) {
        mask0 += (unsigned short)Mask::k_Corner;
    }
    if (IsSharp(true)) {
        mask1 += (unsigned short)Mask::k_Corner;
    }
    ...
    while (edge) {
        ...
        if (edge->IsSharp(false)) {
            if (mask0 < (unsigned short)Mask::k_Corner) {
                mask0++;
            }
        }
        if (edge->IsSharp(true)) {
            if (mask1 < (unsigned short)Mask::k_Corner) {
                mask1++;
            }
        }
        // If any incident edge is semisharp, mark the vertex as volatile
        if (esharp > float(HbrHalfedge<T>::Mask::k_Smooth) && esharp < float(HbrHalfedge<T>::Mask::k_InfinitelySharp)) {
            volatil = 1;
        }
        nextedge = GetNextEdge(edge);
        if (nextedge == start) {
...
HbrVertex<T>::GetFVarMask(int datum) {
    ...
    // to corner
    if (IsSharp(false)) {
        mask += (unsigned char)Mask::k_Corner;
    }
    ...
    while (edge) {
        if (edge->GetFVarSharpness(datum)) {
            if (mask < (unsigned char)Mask::k_Corner) {
                mask++;
            } else {
...
HbrVertex<T>::GetFractionalMask() const {
    ...
    if (sharpness > float(Mask::k_Smooth) && sharpness < float(Mask::k_Dart)) {
        mask += sharpness; ++n;
    }
    ...
    while (edge) {
        float esharp = edge->GetSharpness();
        if (esharp > float(HbrHalfedge<T>::Mask::k_Smooth) && esharp < float(HbrHalfedge<T>::Mask::k_Sharp)) {
            mask += esharp; ++n;
        }
        next = GetNextEdge(edge);
        if (next == start) {
