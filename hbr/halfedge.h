...
HbrHalfedge<T>::IsFVarInfiniteSharpAnywhere() {
    ...
    if (sharpness > float(Mask::k_Smooth)) {
        return true;
    }
...
HbrHalfedge<T>::GetFVarSharpness(int datum, bool ignoreGeometry) {
    ...
    if (GetFVarInfiniteSharp(datum)) return float(Mask::k_InfinitelySharp);
    ...
    if (!ignoreGeometry) {
        ...
        if (sharpness > float(Mask::k_Smooth)) {
            ...
            return float(Mask::k_InfinitelySharp);
        }
    }
    return float(Mask::k_Smooth);
}
