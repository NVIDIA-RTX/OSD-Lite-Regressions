...
HbrLoopSubdivision<T>::Refine(HbrMesh<T>* mesh, HbrFace<T>* face) {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (!face->GetChild(i)) {
            ...
            if ((sharpness = edge->GetSharpness()) > float(HbrVertex<T>::Mask::k_Smooth)) {
            ...
            if ((sharpness = prevedge->GetSharpness()) > float(HbrVertex<T>::Mask::k_Smooth)) {
                HbrSubdivision<T>::SubdivideCreaseWeight(
...
HbrLoopSubdivision<T>::Subdivide(HbrMesh<T>* mesh, HbrVertex<T>* vertex) {
    ...
    if (sharp >= float(HbrVertex<T>::Mask::k_InfinitelySharp)) {
        ...
        v->SetSharpness(std::max(float(HbrVertex<T>::Mask::k_Smooth), sharp - 1.0f));
    } else {
