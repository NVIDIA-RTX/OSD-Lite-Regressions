...
HbrSubdivision<T>::SubdivideCreaseWeight(HbrHalfedge<T>* edge, HbrVertex<T>* vertex, HbrHalfedge<T>* subedge) {
    ...
    if (sharpness >= float(HbrHalfedge<T>::Mask::k_InfinitelySharp)) {
        subedge->SetSharpness(float(HbrHalfedge<T>::Mask::k_InfinitelySharp));
    }
    ...
    // plus 1/4 of crease sharpnesses incident to vertex
        ...
        public:
            ...
            virtual void operator() (HbrHalfedge<T> &edge) {
                ...
                if ((&edge==m_edge) || (&edge==m_edge->GetOpposite()))
                    return;
                if (edge.GetSharpness() > float(HbrHalfedge<T>::Mask::k_Smooth)) {
        ...
        childsharp -= 1.0f;
        if (childsharp < (float) HbrHalfedge<T>::Mask::k_Smooth) {
            childsharp = (float) HbrHalfedge<T>::Mask::k_Smooth;
        }
        subedge->SetSharpness(childsharp);

    } else {
        sharpness -= 1.0f;
        if (sharpness < (float) HbrHalfedge<T>::Mask::k_Smooth) {
            sharpness = (float) HbrHalfedge<T>::Mask::k_Smooth;
        }
